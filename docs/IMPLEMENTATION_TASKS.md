...
次の実装対象は **T-106 REPL 履歴の永続化** とする。  
前回更新: 2026-05-10

理由は次の通り。

1. T-104 までで学習コンテンツへの再到達導線は整ったため、次は REPL 側の継続利用価値を補うのが自然
2. `src/pages/ReplPage.tsx` と `src/utils/storage.ts` を中心に閉じた範囲で実装でき、Phase 1 をほぼ完了まで進められる
3. REPL 履歴はフリーモード利用の再開体験を改善し、問題学習以外の滞在価値も上げられる
4. T-301 のイベント計測や将来の復習導線設計にも再利用しやすい
...
| Phase 1: 継続利用基盤 | 7 (T-101〜T-106, T-102A) | 6 | 86% |
...
| 順番 | ID | タスク名 | 依存 |
|---|---|---|---|
| 1 | T-106 | REPL 履歴の永続化 | なし |
| 2 | T-105 | 学習パスの導入 | T-101, T-102 |
| 3 | T-201 | 問題データに商品属性を追加 | T-105 |
| 4 | T-202 | コース単位の表示設計 | T-201 |
| 5 | T-203 | ロック済みコンテンツ UI の土台 | T-201 |
| 6 | T-301 | イベント計測の抽象化 | T-101, T-104 |
| 7 | T-302 | CTA と価格導線の追加 | T-301 |
| 8 | T-303 | 価格ページの静的実装 | T-302 |
| 9 | T-304 | メール獲得導線 | T-303 |
