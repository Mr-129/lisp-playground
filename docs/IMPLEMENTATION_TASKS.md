...
次の実装対象は **T-104 問題とガイドの検索** とする。  
前回更新: 2026-05-10

理由は次の通り。

1. T-103 までで再開導線は整ったため、次は 51 問 + ガイドを探しやすくする価値が高い
2. `ProblemList` / `LearnPage` / `LispGuide` の既存 UI を活かして、閉じた範囲で実装できる
3. 検索は T-105 学習パスや T-301 計測の前段としても再利用しやすい
4. ブックマーク・最近見た問題と相性が良く、継続利用の導線をさらに強化できる
...
| Phase 1: 継続利用基盤 | 7 (T-101〜T-106, T-102A) | 5 | 71% |
...
| 順番 | ID | タスク名 | 依存 |
|---|---|---|---|
| 1 | T-104 | 問題とガイドの検索 | なし |
| 2 | T-106 | REPL 履歴の永続化 | なし |
| 3 | T-105 | 学習パスの導入 | T-101, T-102 |
| 4 | T-201 | 問題データに商品属性を追加 | T-105 |
| 5 | T-202 | コース単位の表示設計 | T-201 |
| 6 | T-203 | ロック済みコンテンツ UI の土台 | T-201 |
| 7 | T-301 | イベント計測の抽象化 | T-101, T-104 |
| 8 | T-302 | CTA と価格導線の追加 | T-301 |
| 9 | T-303 | 価格ページの静的実装 | T-302 |
| 10 | T-304 | メール獲得導線 | T-303 |
