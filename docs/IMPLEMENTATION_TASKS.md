...
次の実装対象は **T-203 ロック済みコンテンツ UI の土台** とする。  
前回更新: 2026-05-10

理由は次の通り。

1. T-202 でコース別の見せ方が入ったので、次は `tier` を使って無料と有料候補の違いを見せる準備に進める
2. `courseId` と `courseOrder` が UI に出たことで、ロック表示をコース文脈の中で自然に配置できる状態になった
3. 課金導線の前段として「見えるがまだ使えない」体験を整えると、T-301 以降の計測対象も明確になる
4. 静的サイトとしての完成度を上げるうえでも、価格導線に入る前にコンテンツ差分の見せ方を作っておく価値が高い
...
| Phase 1.5: 商品設計前提 | 3 (T-201〜T-203) | 2 | 67% |
...
| 順番 | ID | タスク名 | 依存 |
|---|---|---|---|
| 1 | T-203 | ロック済みコンテンツ UI の土台 | T-201 |
| 2 | T-301 | イベント計測の抽象化 | T-101, T-104 |
| 3 | T-302 | CTA と価格導線の追加 | T-301 |
| 4 | T-303 | 価格ページの静的実装 | T-302 |
| 5 | T-304 | メール獲得導線 | T-303 |
