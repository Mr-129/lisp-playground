...
次の実装対象は **T-202 コース単位の表示設計** とする。  
前回更新: 2026-05-10

理由は次の通り。

1. T-201 で `courseId` と `courseOrder` が入ったため、次はデータを実際の一覧導線へ見せる段階に進むのが自然である
2. いまなら `src/components/ProblemList.tsx` と `src/pages/LearnPage.tsx` の表示変更だけで、カテゴリとコースの両軸を比較的安全に導入できる
3. T-203 のロック UI は、コース表示が先にないと有料候補の見せ方が曖昧になりやすい
4. 静的サイトとしての完成度を上げる意味でも、次は商品属性の内部整備より表示設計の方が体験価値に直結する
...
| Phase 1.5: 商品設計前提 | 3 (T-201〜T-203) | 1 | 33% |
...
| 順番 | ID | タスク名 | 依存 |
|---|---|---|---|
| 1 | T-202 | コース単位の表示設計 | T-201 |
| 2 | T-203 | ロック済みコンテンツ UI の土台 | T-201 |
| 3 | T-301 | イベント計測の抽象化 | T-101, T-104 |
| 4 | T-302 | CTA と価格導線の追加 | T-301 |
| 5 | T-303 | 価格ページの静的実装 | T-302 |
| 6 | T-304 | メール獲得導線 | T-303 |
