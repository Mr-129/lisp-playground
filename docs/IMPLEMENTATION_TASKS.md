...
次の実装対象は **T-301 イベント計測の抽象化** とする。  
前回更新: 2026-05-10

理由は次の通り。

1. T-203 までで静的サイト上の学習導線と商品表示の下地が揃ったため、次は利用計測を入れて仮説検証できる状態に進めるのが自然である
2. 問題閲覧、検索、REPL 利用、エディタ実行など主要イベントはすでに UI 上に揃っており、今の段階で計測点を整理しやすい
3. T-302 以降の CTA や価格導線は、先に `trackEvent` 相当の薄い基盤がないと後から計測整理が発生しやすい
4. 収益化前の静的サイトとしても、まず行動データを取れる状態にする方が優先度が高い
...
| 順番 | ID | タスク名 | 依存 |
|---|---|---|---|
| 1 | T-301 | イベント計測の抽象化 | T-101, T-104 |
| 2 | T-302 | CTA と価格導線の追加 | T-301 |
| 3 | T-303 | 価格ページの静的実装 | T-302 |
| 4 | T-304 | メール獲得導線 | T-303 |
