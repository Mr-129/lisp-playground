...
1. **評価モデルと quote 系の厚みを増やす**
2. **tree / alist を中心に Lisp 的データ処理を増やす**
3. **等価性、述語、型分岐の判断問題を増やす**
4. **状態変化、scope、closure の誤解しやすい箇所を増やす**
5. **読解 / デバッグ / 修正型問題を追加する**
...
最初の 1 バッチとしては、次の 5 テーマが妥当である。

1. quote / `#'` / `function` / `funcall` / `apply`
2. tree 再帰と `assoc`
3. `eq` / `eql` / `equal` と真偽値
4. `let` / `let*` / `setf` / closure
5. 読解 / デバッグ / エラー修正
