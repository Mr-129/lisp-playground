...
本書は、Lisp Playground の競合と代替手段を整理し、今後の問題追加、学習導線、商品設計の参考にするための比較メモである。
...
これらは、学習順序や演習をある程度提供しており、Lisp Playground と比較されやすい。
